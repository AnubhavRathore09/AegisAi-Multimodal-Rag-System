...
from src.services.news import get_live_news
...
def _clean_prompt_template(prompt_template: str) -> str:
    return prompt_template if prompt_template in settings.prompt_templates else "generate_prompt"
...
def _build_prompt(query: str, context: str, prompt_template: str, memory_text: str, bot_name: str) -> str:
    style = {
        ...
        "generate_prompt": "Use short readable paragraphs and bullets only when they help.",
        "live_news_prompt": "Lead with the latest developments, keep the answer grounded in the live news context, and call out uncertainty when details are still developing.",
...
async def _plan_pipeline(request: ChatRequest, user_id: str | None = None) -> PipelineResult:
    ...
    if route == "search":
        prompt_template = "live_news_prompt"
    ...
    if route == "search" and not context:
        news_context = await asyncio.to_thread(get_live_news, query)
        context = _trim_context(news_context)
        retrieval = {
            "matches": 1 if context else 0,
            "hybrid_search": False,
            "context_chars": len(context),
            "top_score": 1.0 if context else 0.0,
        }
        citations = ([{
            "source": "NewsData.io",
            "kind": "live_news",
        ...
        if not context:
            warnings.append("live_news_empty")

    if decision.use_retrieval and route != "search" and not context:
